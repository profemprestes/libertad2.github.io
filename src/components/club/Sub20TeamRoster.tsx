/**
 * @file Sub20TeamRoster.tsx
 * @description Component to display the Sub-20 team roster.
 * It groups players by position (Arquero, Defensa, Volante, Delantero) 
 * and sorts players alphabetically within each position.
 * Uses Sub20PlayerCard to display individual player details.
 */
import type { Sub20Player } from '@/types';
import { Sub20PlayerCard } from './Sub20PlayerCard';

interface Sub20TeamRosterProps {
  players: Sub20Player[];
}
...
export function Sub20TeamRoster({ players }: Sub20TeamRosterProps) {
  if (!players || players.length === 0) {
    return <p className="text-center text-muted-foreground">No hay jugadores Sub 20 para mostrar.</p>;
  }

  // Group players by position
  const groupedPlayers: Record<string, Sub20Player[]> = players.reduce((acc, player) => {
          ...
          <section key={position} className="mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {playersInPosition.map((player) => (
                <Sub20PlayerCard key={player.nombre} player={player} />
              ))}
            </div>
          </section>
        );
